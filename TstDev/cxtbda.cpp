...
HRESULT CBdaGraph::DVBS_Conexant_DiSEqC(BYTE len, BYTE *DiSEqC_Command)
{
	...
	if(FAILED(hr))
	{
		sprintf(text,"BDA2: DVBS_Conexant_DiSEqC: failed sending DISEQC_MESSAGE_PARAMS (0x%8.8x)", hr);
		ReportMessage(text);
		return E_FAIL;
	}
	sprintf(text,"BDA2: DVBS_Conexant_DiSEqC: sent DISEQC_MESSAGE_PARAMS");
	ReportMessage(text);
...
HRESULT CBdaGraph::DVBS_Bestunar_DiSEqC(BYTE len, BYTE *DiSEqC_Command)
{
	...
	if(FAILED(hr))
	{
		sprintf(text,"BDA2: DVBS_Bestunar_DiSEqC: failed sending DISEQC_MESSAGE_PARAMS (0x%8.8x)", hr);
		ReportMessage(text);
		return E_FAIL;
	}
	sprintf(text,"BDA2: DVBS_Bestunar_DiSEqC: sent DISEQC_MESSAGE_PARAMS");
	ReportMessage(text);
...
HRESULT CBdaGraph::DVBS_Conexant_LNBPower(BOOL bPower)
{
	...
	if(FAILED(hr))
	{
		sprintf(text,"BDA2: DVBS_Conexant_LNBPower: failed set LNB Power state (0x%8.8x)", hr);
		ReportMessage(text);
		return E_FAIL;
	}
	sprintf(text,"BDA2: DVBS_Conexant_LNBPower: set LNB Power state");
	ReportMessage(text);

...
extern int (*TeVii_GetAPIVersion)();
...
extern BOOL (*TeVii_OpenDevice)(int idx, lpCapFunc func, void* lParam);

//Close Device
//	params:
//		idx - device index of previously opened device (0 <= idx < FindDevices())
//	result: non-zero on success
extern BOOL (*TeVii_CloseDevice)(int idx);
...
extern BOOL (*TeVii_TuneTransponder)(int idx, DWORD Freq, DWORD SymbRate, DWORD LOF, TPolarization Pol, BOOL  F22KHz, TMOD MOD, TFEC FEC);
...
//	result: non-zero on success
extern BOOL (*TeVii_GetSignalStatus)(int idx, BOOL* IsLocked, DWORD* Strength, DWORD* Quality);
...
//	result: non-zero on success
extern BOOL (*TeVii_SendDiSEqC)(int idx, BYTE* Data, DWORD Len, DWORD Repeats, BOOL Flg);
...
//	result: non-zero on success
extern BOOL (*TeVii_SetRemoteControl)(int idx, lpRCFunc lpCallback, void* lParam);
